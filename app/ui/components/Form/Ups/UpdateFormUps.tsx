...
const UpdateFormUps: FC<UpdateFormProps> = ({
    id,
}) => {
  const [upsData, setUpsData] = useState<UpdateScanner_Ups>({
    id:id,
    direccion: "",
    departamento: "",
    observaciones: "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getUpsById(id);
        setUpsData(response);
      } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.flag === false) {
        ...
        toast.success("Ups Actualizado Correctamente");
        setTimeout(() => {window.location.reload();},2000);
      }
    } catch (error) {
      console.error("Error Actualizando ups:", error);
    }
  };
    ...
    <div>
      ...
      <DialogTrigger >
        ...
        <DialogContent className="sm:max-w-[525px]">
          <DialogHeader>
            <DialogTitle>Editar Ups</DialogTitle>
            <DialogDescription>Introduzca los datos del Ups que quiere editar.</DialogDescription>
          </DialogHeader>
          ...
          <DialogClose asChild>
            ...
            <Button type="submit" className="relative left-[120px]">Actualizar Ups</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
