...
import { EquipoWithPrograma, Flotas, Monitor, Other, UpdateEquipo, ViewFlotas } from "@/types/indes";
...
const ViewFormFlotas: FC<ViewFormProps> = ({
    serial
}) => {
  const [equipoData, setEquipoData] = useState<ViewFlotas>({
    id: 0,
    tipo: "",
    marca: "",
    modelo: "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<ViewFlotas>(`${apiUrl}/Flotas/BySerial/${serial}`);
      ...
      <DialogTrigger >
        ...
        <DialogContent className="sm:max-w-[825px]">
          ...
          <form >
            ...
            <div className="">
                <Label htmlFor="datos" className="flex text-right justify-between">
                  Plan de datos : {equipoData.datos}
                </Label>
            </div>
            <div className="">
