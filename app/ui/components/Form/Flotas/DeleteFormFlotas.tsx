...
import 'react-toastify/dist/ReactToastify.css';
  ...
  const DeleteDialogFlotas: FC<DeleteFormProps> =({
    id,
}) => {
  ...
  const session = useSession();
    ...
    const handleSubmit = async () => {
        try {
          ...
          if (result.flag === false) {
            ...
            toast.success("Flota eliminada exitosamente");
            setTimeout(() => {window.location.reload();},2000);
          }
        } catch (error) {
          console.error("Error Eliminando flota:", error);
        }
      };
    ...
    <AlertDialogContent>
        <AlertDialogHeader>
        <AlertDialogTitle>Estas seguro que quieres eliminar esta flota?</AlertDialogTitle>
        <AlertDialogDescription>
        Esta acción no se puede deshacer. Esto eliminará permanentemente 
        la flota y eliminará sus datos de nuestros servidores.
        </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export default DeleteDialogFlotas;