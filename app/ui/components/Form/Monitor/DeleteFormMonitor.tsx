...
import 'react-toastify/dist/ReactToastify.css';
  ...
  const DeleteDialogMonitor: FC<DeleteFormProps> =({
    id,
}) => {
  ...
  const session = useSession();
    ...
    const handleSubmit = async () => {
        try {
          const result = await DeleteMonitor(id, session.data?.accessToken);
    ...
    <AlertDialogContent>
        <AlertDialogHeader>
        <AlertDialogTitle>Estas seguro que quieres eliminar este monitor?</AlertDialogTitle>
        <AlertDialogDescription>
        Esta acción no se puede deshacer. Esto eliminará permanentemente 
        el Monitor y eliminará sus datos de nuestros servidores.
        </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
export default DeleteDialogMonitor;