...
import { CreateUps } from "@/services/equipos/ups";
...
const CreateFormUps: FC<CreateFormProps> = () => {
  const [upsData, setUpsData] = useState<CreateScanner_Ups>({
    marca: "",
    modelo: "",
    serial: "",
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await CreateUps(upsData, session.data?.accessToken);
      if (result.flag === false) {
        toast.error(result.message);
        console.log(result.message);
      }
      else {
        toast.success("Ups creado exitosamente");
  ...
  return (
    <div>
      ...
      <DialogTrigger asChild>
        <Button variant="outline" >Agregar Ups</Button>
      </DialogTrigger>
        <DialogContent className=" sm:max-w-[925px] ">
          <DialogHeader>
            <DialogTitle>Agregar Ups</DialogTitle>
            <DialogDescription>Introduzca los datos del Ups.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} >
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                value={upsData.codigo_Bienes_Nacionales}
                onChange={handleChange}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input
                ...
                value={upsData.codigo_Invi_Mived}
                onChange={handleChange}
                className="col-span-3"
              />
            </div>
